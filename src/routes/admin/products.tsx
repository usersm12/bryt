        ...
        <table className="w-full text-sm">
          <thead className="border-b border-slate-100 bg-slate-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 hidden md:table-cell">Category / Group</th>
              ...
              <tr key={p.slug} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3 hidden lg:table-cell">
                  <div className="flex gap-1.5 flex-wrap">
                    {p.tagline && <span className="rounded-full bg-green-50 px-2 py-0.5 text-xs text-green-700">Rich details</span>}
                    {p.image_url && <span className="rounded-full bg-blue-50 px-2 py-0.5 text-xs text-blue-700">Image</span>}
                    {!p.tagline && !p.image_url && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-400">Basic</span>}
                  </div>
                </td>
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <Link to="/admin/products/$id" params={{ id: p.slug }}
